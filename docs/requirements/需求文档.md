...
> **映射说明**：追溯关系在API层使用简单type字符串，服务层映射到具体EClass：
>
> - API: `POST /api/v1/traces {"source", "target", "type"}`
> - 服务层映射：
>   - `type="derive"` → 创建DeriveRequirement实例
>   - `type="satisfy"` → 创建Satisfy实例  
>   - `type="refine"` → 创建Refine实例
>   - `type="trace"` → 创建Trace实例
...
- **REQ-C3-4** 追溯语义约束（最小规则）
  - **AC**：
    ...
    - `Verify`: **P2阶段实现**，MVP不包含TestCase/VerificationCase
...
- **T-TRACE-MAP**：创建type=satisfy后，模型内存在Satisfy实例；删除trace时对应EClass实例被移除
