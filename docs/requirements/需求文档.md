...
- **REQ-D1-1** 树视图数据构建
  - **AC**：前端通过 `GET /api/v1/elements` 获取所有元素，在客户端构建树结构；Definition 为父节点，相关 Usage（通过 `of` 字段关联）为子节点；树视图为SSOT的投影，不调用专门接口。
- **REQ-D1-2** 写回
  - **AC**：创建/重命名/拖拽改层级通过 `POST/PATCH /api/v1/elements` 更新；非法层级变更 → `400`。
- **REQ-D1-3** 联动
