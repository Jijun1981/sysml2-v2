...
- **REQ-B5-3** 通用元素接口（完整CRUD）
  - **AC**：`/api/v1/elements`提供完整CRUD功能，支持所有182种SysML类型；UniversalElementService作为基础设施层，被领域服务通过委托模式调用；领域特定接口（/api/v1/requirements等）内部委托给UniversalElementService执行EMF操作。
- **REQ-B5-4** 扩展新类型
